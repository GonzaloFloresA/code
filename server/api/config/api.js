API = {
  authentication: function( apiKey ) {
    var getUser = APIKeys.findOne( { "key": "ad83e3a185fc24b2dbf173e7389b1ff9"}, { fields: { "owner": "3Ywsqs7ZpaDpqizCk"} } );
    if ( getUser ) {
    ...
    pizza: {
      GET: function( context, connection ) {
        ...
        if ( hasQuery ) {
          ...
          var getPizzas = APIKeys.find( connection.data ).fetch();
          ...
          // Our request didn't cSontain any params, so we'll just return all of
          // the pizzas we have for the owner associated with the passed API key.
          var getPizzas = APIKeys.find( { "owner": connection.owner } ).fetch();
          //var getPizzas = Pizza.find().fetch(); ...cambie esto x arreglar falla degenerar keys
          API.utility.response( context, 200, getPizzas );
        }
      },
